...
pub enum Driving {
    /// Independent reference executor; alpha slot reloads do not replace it.
    Wbc,
    Walk,
    Stand,
...
impl Controller {
    ...
    /// A policy-slot reload replaces the alpha controller, not the independent WBC
    /// executor. Move it intact so reload cannot silently drop an active reference.
    pub fn take_wbc_from(&mut self, from: &mut Controller) {
        self.wbc = from.wbc.take();
        self.wbc_active = from.wbc_active;
        self.wbc_transition = from.wbc_transition;
    }
    ...
    pub fn step(
        ...
        if self.wbc_active {
            ...
            return Ok(Step {
                ...
                label: if step.finished { "wbc_final" } else { "wbc" }.into(),
                gain: step.gain,
                busy: true,
    ...
    fn wbc_runs_through_the_existing_skill_controller() {
        ...
        let policy = Policy::load(
            &duck_control::policy::PolicyPaths {
                walk: policies.join("../duck-control/tests/fixtures/feedforward.onnx"),
                ..Default::default()
            },
        ...
        for frame in 0..989 {
            if frame == 100 {
                ...
                .unwrap();
                let mut replacement =
                    Controller::new(policy, Tuning::default(), SkillTuning::default());
                replacement.carry_over(&controller);
                replacement.take_wbc_from(&mut controller);
                controller = replacement;
                assert_eq!(controller.driving(), Some(Driving::Wbc));
            }
            let step = controller
                .step(
                    &duck_control::Sensors::default(),
