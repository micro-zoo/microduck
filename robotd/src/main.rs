...
use control::{Controller, Driving, Error as ControlError, SkillTuning, Tuning, WbcTransition};
use intents::Intents;
...
fn change_disturbs(
    ...
    // Alpha slot reloads cannot change WBC: its executor moves intact to the new controller.
    if matches!(driving, Driving::Seated | Driving::Wbc) {
        return false;
    }
    ...
    match driving {
        ...
        Driving::Seated | Driving::Wbc => false,
            ...
            && let Some(swap) = pending_swap.take_if(|p| p.ready())
        {
            ...
            match built.expect("take_if checked the load has finished") {
                Ok(mut built) => {
                    ...
                    if let (Some(new), Some(old)) = (built.as_mut(), controller.as_mut()) {
                        new.take_wbc_from(old);
                    }
                    controller = built;
            ...
            (true, Some(sensors)) => {
                ...
                match controller.step(sensors, &command, snapshot.pose.active, dt, scale_mult) {
                    ...
                    Err(ControlError::Wbc(e)) => {
                        ...
                        (sensors.positions, policy_cfg.gain, true, "wbc_fault".into())
                    }
                    Err(e) => {
...
fn apply_intent(state: &RobotState, intents: &Intents, call: &proto::Call) -> bool {
    match call {
        ...
        proto::Call::RobotDo(p) => {
            if p.skill == "wbc_toggle" {
                request_wbc(state, intents);
            } else {
...
fn pad_bindings_report(config: &std::path::Path, state: &RobotState) -> proto::PadBindingsResult {
    ...
    let known = available_do_names(state);
...
fn available_do_names(state: &RobotState) -> Vec<String> {
    let mut names = do_names(&state.policies.load());
    if state.wbc_available.load(Ordering::Relaxed) {
        names.push("wbc_toggle".into());
    }
    names
}
...
fn request_wbc(state: &RobotState, intents: &Intents) -> proto::IntentResult {
    if state.wbc_switching.load(Ordering::Relaxed) {
        return proto::IntentResult::refused("a WBC skill switch is already in flight");
    }
    if !state.wbc_available.load(Ordering::Relaxed) {
        ...
        return proto::IntentResult::refused(reason);
    }
    if !state.wbc_active.load(Ordering::Relaxed) {
        if mode_of(state.mode.load(Ordering::Relaxed)) != Mode::Walk {
            return proto::IntentResult::refused("WBC is available only in walking mode");
        }
        if !intents.enabled() {
            return proto::IntentResult::refused("enable the policy before switching to WBC");
        }
        if !state.homed.load(Ordering::Relaxed) {
            return proto::IntentResult::refused("wait until the robot is homed");
        }
        if state.moving.load(Ordering::Relaxed) {
            return proto::IntentResult::refused("stop the robot before switching to WBC");
        }
    }
    intents.request_wbc_toggle();
    proto::IntentResult::accepted()
}
...
fn dispatch(
    ...
    match call {
        ...
        proto::Call::RobotDo(p) => {
            ...
            if p.skill == "wbc_toggle" {
                return proto::Response::ok(Some(id), &request_wbc(state, intents));
            }
            let result = if state.wbc_active.load(Ordering::Relaxed)
                || state.wbc_switching.load(Ordering::Relaxed)
            {
...
mod tests {
    ...
    #[test]
    fn alpha_reload_leaves_the_independent_wbc_executor_running() {
        let before = Params::default().policy.resolved();
        let mut after = before.clone();
        after.walk = PathBuf::from("/new/walk.onnx");
        for change in [
