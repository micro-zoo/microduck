...
impl SkillRequests {
    pub fn any(&self) -> bool {
        self.wbc_toggle || self.ground_pick || self.sit_toggle || self.skills != 0
    }
...
pub struct Intents {
    ...
    // Separate from the full 32-bit upstream skill mask.
    wbc_toggle: AtomicBool,
...
impl Intents {
    pub fn new() -> Self {
        ...
        Self {
            ...
            wbc_toggle: AtomicBool::new(false),
    ...
    pub fn request_wbc_toggle(&self) {
        self.wbc_toggle.store(true, Ordering::Relaxed);
    }
    ...
    pub fn take_skills(&self) -> SkillRequests {
        ...
        SkillRequests {
            wbc_toggle: self.wbc_toggle.swap(false, Ordering::Relaxed),
...
mod tests {
    ...
    #[test]
    fn wbc_does_not_collide_with_any_configurable_skill_bit() {
        let intents = Intents::new();
        intents.request_wbc_toggle();
        for i in 0..MAX_SKILLS {
            intents.request_skill(i);
        }
        let requests = intents.take_skills();
        assert!(requests.wbc_toggle);
        assert_eq!(
            requests.requested().collect::<Vec<_>>(),
            (0..MAX_SKILLS).collect::<Vec<_>>()
        );
        assert!(!intents.take_skills().any());
    }
