...
enum Command {
    ...
    Package {
        ...
        /// Extra files or directories to include, as `src=dest` (e.g. a hook or web assets).
        #[arg(long = "include")]
        includes: Vec<String>,
    ...
    if metadata.is_dir() {
        ...
        for child in children {
            let name = child
                .file_name()
                .into_string()
                .map_err(|_| "unreadable include name")?;
            // Included directory trees contain static resources, not executable scripts.
